...
```
In Spinnaker version 1.36, there's a new set of defaults that replace the default timeouts on internal calls to Front50 with drastically lower defaults. Further, these defaults now ignore any overridden OkHttp settings for inter-service communications.

This can break certain operations that might have previously worked due to the lower timeout defaults. The most common case is when service accounts are created as part of saving a pipeline.

As such, you should adjust your Front50 timeout calls explicitly for your use case using these different properties:
```yaml
    front50:
      okhttp:
